# Alien Cat OS — 이어가기 프롬프트 (HANDOFF)
...
너는 내 블로그 프로젝트 "Alien Cat OS"(구 404 Tacocat OS)를 이어서 작업해야 해. 아래 상태를 읽고 그대로 이어가줘.

## 🎯 프로젝트
- **Alien Cat OS**: 레트로 데스크톱 OS 컨셉 픽셀아트 포트폴리오 블로그 (2026-07-14 리브랜딩 완료)
...
## ✅ 완료된 것
- **Alien Cat OS 리브랜딩 (2026-07-14 배포)**: 마스코트 = 우주복 입은 초록 외계고양이(`public/mascot.svg`, 24×28 도트 — 도트 원본은 wallpaper 시안 이력 참고), 배경 = 사계절 우주(달 위 부유 마스코트 + 24×24 지구 + 은하수·성운·별 블링크, `src/scripts/wallpaper.ts` 캔버스 렌더러), 파비콘/OG 이미지/부팅(v2.0 space edition)/404 문구까지 전부 교체. 아이콘·창 UI는 기존 코지 톤 유지.
- **OS UI 전체**: 부팅(첫 방문만)/드래그 창/작업표시줄/시작메뉴/Esc/날짜시계, 사계절 우주 배경(`?season=` 미리보기)
...
- 코드 구조: 클라이언트 JS=`src/scripts/desktop.ts`+`wallpaper.ts`(우주 배경 렌더러), 아이콘=`src/config/icons.ts`, CSS=`src/styles/`(base/os/wallpaper/prose, pixel.css 허브)
...
4. 도메인 구매 — 리브랜딩으로 재검토 필요 (기존 후보 404tacocatblog.com은 Vercel에 이름만 등록됨 → aliencat 계열로 바꿀지 사용자 결정 대기)
...
- **노션 이미지는 임시 URL(5분~1시간 만료)** — fetch 직후 바로 다운로드해서 `src/assets/study/<슬러그>/`에 저장하고 마크다운은 상대경로 참조(Astro가 WebP 변환, 상세 STUDY-SYNC §6). 받은 뒤 **이미지를 실제로 열어 내용↔캡션 일치 검수** (순서 뒤바뀜/중복 사례 있었음)
